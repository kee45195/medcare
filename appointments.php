...
<body>
            ...
            <div class="tab-pane fade show active" id="upcoming" role="tabpanel">
                ...
                <?php else: ?>
                    <?php foreach ($upcoming_appointments as $appointment):
                        ...
                        $doctor_working_days_raw = (string)($appointment['working_days'] ?? '');
                        $doctor_working_days_array = array_values(array_filter(parseWorkingDays($doctor_working_days_raw), 'strlen'));
                        $doctor_working_days_display = !empty($doctor_working_days_array) ? implode(', ', $doctor_working_days_array) : 'Not specified';
                        $doctor_working_hours_raw = trim((string)($appointment['working_hours'] ?? ''));
                        $doctor_working_hours_display = $doctor_working_hours_raw !== '' ? $doctor_working_hours_raw : 'Not specified';
                    ?>  
                            ...
                            <div class="card-body">
                                ...
                                <div class="appointment-actions">
                                    <button type="button"
                                            ...
                                            data-working-days="<?php echo htmlspecialchars(json_encode($doctor_working_days_array), ENT_QUOTES, 'UTF-8'); ?>"
                                            data-working-days-display="<?php echo htmlspecialchars($doctor_working_days_display, ENT_QUOTES, 'UTF-8'); ?>"
                                            data-working-hours-display="<?php echo htmlspecialchars($doctor_working_hours_display, ENT_QUOTES, 'UTF-8'); ?>"
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" class="form-control" name="new_date" id="new_date" min="<?php echo date('Y-m-d'); ?>" required>
                             <div class="form-text" id="new_date_help"></div>
                            <div class="invalid-feedback d-none" id="new_date_invalid">Please select a valid working day for the doctor.</div>
                        </div>
                        <div class="mb-3">
    ...
    <script>
        ...
        const newDateHelp = document.getElementById('new_date_help');
        const newDateInvalid = document.getElementById('new_date_invalid');
        const workingDaysDisplayEl = document.getElementById('reschedule_working_days');
        const workingHoursDisplayEl = document.getElementById('reschedule_working_hours');
        ...
        function ensureDateMatchesAvailability(preferredDateStr, allowedDays) {
            ...
            let candidate = parseDateString(preferredDateStr);
        ...
        function getAllowedDaysFromInput() {
            if (!newDateInput) {
                return [];
            }
            return (newDateInput.dataset.allowedDays || '')
                .split(',')
                .map(function (day) { return day.trim(); })
                .filter(function (day) { return day.length > 0; });
        }

        function validateDateAgainstAllowedDays() {
            if (!newDateInput) {
                return;
            }

            const allowedDays = getAllowedDaysFromInput();
            const selectedValue = newDateInput.value;

            if (!allowedDays.length) {
                newDateInput.setCustomValidity('');
                newDateInput.classList.remove('is-invalid');
                if (newDateInvalid) {
                    newDateInvalid.classList.add('d-none');
                }
                return;
            }
            ...
            if (!selectedDate) {
                newDateInput.setCustomValidity('Please select a valid date.');
                newDateInput.classList.add('is-invalid');
                if (newDateInvalid) {
                    newDateInvalid.textContent = 'Please select a valid date.';
                    newDateInvalid.classList.remove('d-none');
                }
                return;
            }
            ...
            if (!allowedDays.includes(dayName)) {
                const guidance = allowedDays.join(', ');
                newDateInput.setCustomValidity('Doctor is unavailable on ' + dayName + '.');
                newDateInput.classList.add('is-invalid');
                if (newDateInvalid) {
                    newDateInvalid.textContent = guidance ? 'Please choose a date on: ' + guidance : 'Selected day is unavailable.';
                    newDateInvalid.classList.remove('d-none');
                }
            } else {
                newDateInput.setCustomValidity('');
        ...
        if (newDateInput) {
            newDateInput.addEventListener('change', validateDateAgainstAllowedDays);
            newDateInput.addEventListener('input', validateDateAgainstAllowedDays);
        }

        function openRescheduleModal(trigger) {
            if (!trigger || !newDateInput) {
                return;
            }

            const appointmentId = trigger.getAttribute('data-appointment-id');
            const currentDate = trigger.getAttribute('data-current-date') || '';
            const currentTime = trigger.getAttribute('data-current-time') || '';
            const workingDaysJson = trigger.getAttribute('data-working-days') || '[]';

            let workingDays = [];
            try {
                const parsed = JSON.parse(workingDaysJson);
            ...
            const workingDaysDisplay = trigger.getAttribute('data-working-days-display') || '';
            const workingHoursDisplay = trigger.getAttribute('data-working-hours-display') || '';
            const availableDaysText = normalizedWorkingDays.length ? normalizedWorkingDays.join(', ') : '';

            document.getElementById('reschedule_appointment_id').value = appointmentId;
            
            document.getElementById('new_time').value = currentTime;
             if (workingDaysDisplayEl) {
                workingDaysDisplayEl.textContent = availableDaysText || workingDaysDisplay || 'Not specified';
            }
            if (workingHoursDisplayEl) {
                workingHoursDisplayEl.textContent = workingHoursDisplay || 'Not specified';
            }
            if (newDateHelp) {
                if (availableDaysText) {
                    newDateHelp.textContent = 'Available days: ' + availableDaysText;
                } else {
                    newDateHelp.textContent = 'Doctor availability is not specified.';
                }
            }
            if (newDateInvalid) {
                newDateInvalid.classList.add('d-none');
            ...
            if (normalizedWorkingDays.length) {
                newDateInput.dataset.allowedDays = normalizedWorkingDays.join(',');
            } else {
                newDateInput.dataset.allowedDays = '';
            }

            const adjustedDate = ensureDateMatchesAvailability(currentDate, normalizedWorkingDays);
            newDateInput.value = adjustedDate || '';

            validateDateAgainstAllowedDays();
