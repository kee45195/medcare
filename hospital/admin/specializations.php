...
/* =========================
   CREATE (Name only)
   ========================= */
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_specialization'])) {
    try {
        require_csrf();
        $name = trim($_POST['name'] ?? '');
        if ($name === '') throw new Exception('Specialization name is required.');

        $check = $pdo->prepare("SELECT id FROM specializations WHERE name = ?");
        $check->execute([$name]);
        if ($check->fetch()) throw new Exception('A specialization with this name already exists.');

        $pdo->prepare("INSERT INTO specializations (name) VALUES (?)")->execute([$name]);

        $success_message = 'Specialization created successfully!';
        $action = 'list';
    } catch (Exception $e) {
...
/* =========================
   UPDATE (Name only)
   ========================= */
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_specialization'], $_POST['id'])) {
    try {
        require_csrf();
        $id   = (int)$_POST['id'];
        $name = trim($_POST['name'] ?? '');
        ...
        $check = $pdo->prepare("SELECT id FROM specializations WHERE name = ? AND id <> ?");
        $check->execute([$name, $id]);
        if ($check->fetch()) throw new Exception('Another specialization with this name already exists.');

        $pdo->prepare("UPDATE specializations SET name = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?")
            ->execute([$name, $id]);
...
/* =========================
   DELETE
   ========================= */
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_specialization'], $_POST['id'])) {
    try {
        require_csrf();
        $id = (int)$_POST['id'];
        // doctors.specialization_id has ON DELETE SET NULL FK (per your schema)
        $pdo->prepare("DELETE FROM specializations WHERE id = ?")->execute([$id]);

        $success_message = 'Specialization deleted successfully!';
        $action = 'list';
    } catch (Exception $e) {
        $error_message = 'Error deleting specialization: ' . $e->getMessage();
        $action = 'list';
    }
}
...
try {
    ...
    } elseif ($action === 'edit' && $spec_id) {
        $stmt = $pdo->prepare("SELECT id, name FROM specializations WHERE id = ?");
        $stmt->execute([$spec_id]);
        $editing = $stmt->fetch();
        if (!$editing) {
            $error_message = 'Specialization not found.';
            $action = 'list';
        }
    }
} catch (PDOException $e) {
    $error_message = 'Error loading specialization data.';
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Specializations - <?php echo APP_NAME; ?></title>
...
<body>
   ...
   <div class="sidebar">
        ...
        <nav class="nav flex-column">
            ...
            <a class="nav-link active" href="content.php"><i class="fas fa-edit"></i>Content Management</a>
    ...
    <div class="main-content">
        ...
        <div class="top-navbar d-flex justify-content-between align-items-center">
            <h3 class="mb-0"><i class="fas fa-building me-2"></i><?php echo $action === 'create' ? 'Create Specialization' : ($action === 'edit' ? 'Edit Specialization' : 'Specializations'); ?></h3>
            <div>
                <?php if ($action === 'list'): ?>
                    <a href="?action=create" class="btn btn-light"><i class="fas fa-plus me-2"></i>Create Specialization</a>
                <?php else: ?>
                    <a href="specializations.php" class="btn btn-light"><i class="fas fa-arrow-left me-2"></i>Back to List</a>
                <?php endif; ?>
            </div>
        </div>
        ...
        <div class="content-area">
            ...
            <?php if ($action === 'create' || ($action === 'edit' && $editing)): ?>
                <div class="content-card">
                    <h5 class="mb-4"><i class="fas fa-pen-to-square me-2"></i><?php echo $action === 'create' ? 'New Specialization' : 'Edit Specialization'; ?></h5>
                    <form method="POST">
                        ...
                        <div class="text-center">
                            <?php if ($action === 'create'): ?>
                                <button type="submit" name="create_specialization" class="btn btn-primary btn-lg px-5">
                                    <i class="fas fa-plus me-2"></i>Create
                                </button>
                            <?php else: ?>
                                <button type="submit" name="update_specialization" class="btn btn-primary btn-lg px-5">
                                    <i class="fas fa-save me-2"></i>Save Changes
                                </button>
                                <a href="specializations.php" class="btn btn-outline-secondary btn-lg px-5 ms-3">
            ...
            <?php else: ?>
                <div class="content-card">
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        <h5 class="mb-0"><i class="fas fa-list me-2"></i>All Specializations (<?php echo count($specializations); ?>)</h5>
                        <input type="text" class="form-control" id="searchSpecs" placeholder="Search specializations..." style="width:250px;">
                    </div>
                                ...
                                <?php if (empty($specializations)): ?>
                                    <tr>
                                        <td colspan="3" class="text-center py-4">
                                            ...
                                            <p class="text-muted mb-0">No specializations found.</p>
                                        </td>
                                    </tr>
                                <?php else: ?>
                                    <?php foreach ($specializations as $s): ?>
                                        <tr>
                                            ...
                                            <td class="text-end actions-cell">
                                                <a href="?action=edit&id=<?php echo (int)$s['id']; ?>"
    ...
    <script>
        ...
        (function(){
            const searchInput = document.getElementById('searchSpecs');
